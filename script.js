...
async function submitQuestion(event) {
    ...
    try {
        ...
        const response = await fetch("http://15.206.147.55:8080/submitQuestion", {
            method: "POST",
            body: formData
        });
...
// Function to fetch and display recent questions from the server
async function fetchQuestions() {
    try {
        // Send a GET request to the server to fetch recent questions
        const response = await fetch("http://15.206.147.55:8080/getQuestions");
